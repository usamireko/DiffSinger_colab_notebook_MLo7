...
@click.option(
    '--vocoder', type=click.Path(
        exists=True, file_okay=True, dir_okay=False, readable=True,
        path_type=pathlib.Path, resolve_path=True
    ),
    required=False,
    help='Path to custom vocoder checkpoint (overrides config.yaml)'
)
def acoustic(
        proj: pathlib.Path,
        exp: str,
    ...
    # Override vocoder path if provided
    if vocoder is not None:
        hparams['vocoder_ckpt'] = str(vocoder)
        print(f'| Using custom vocoder: {vocoder}')

    # Check for vocoder path
    vocoder_path = Path(hparams['vocoder_ckpt'])
    if not vocoder_path.is_absolute():
        vocoder_path = root_dir / vocoder_path
    
    assert mel or vocoder_path.exists(), \
        f'Vocoder ckpt \'{hparams["vocoder_ckpt"]}\' not found. ' \
        f'Please put it to the checkpoints directory or specify a custom path with --vocoder.'
